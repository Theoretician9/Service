...
ПРАВИЛА:
- SMART-цель: конкретная сумма/результат + конкретный способ заработка + конкретный срок. Пример: "Заработать 200 000₽ за 3 месяца на продаже саженцев сосен через Авито и местные рынки".
...
Ответ верни СТРОГО в JSON формате (без markdown-обёртки):
{
  ...
  "action_plan": [
    {"week": "Неделя 1", "actions": ["конкретное действие с деталями"]},
    {"week": "Неделя 2", "actions": ["конкретное действие"]},
    {"week": "Неделя 3", "actions": ["конкретное действие"]},
    {"week": "Неделя 4", "actions": ["конкретное действие"]}
  ],
