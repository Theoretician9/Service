...
КРИТИЧЕСКИ ВАЖНО — ПРИОРИТЕТ НИШ:

1. ТЕКУЩИЕ ИНТЕРЕСЫ И НАВЫКИ — в первую очередь:
   - Если человек СЕЙЧАС занимается чем-то (делает ботов, рисует, пишет) → это главный источник ниш
   - Если он пришёл с конкретной идеей (хочу делать ботов) → большинство ниш должны быть ВОКРУГ этой идеи, но для разных аудиторий/сегментов
   - Текущие навыки + текущие интересы → ПРИОРИТЕТ над прошлым опытом

2. ПРОШЛЫЙ ОПЫТ — использовать ТОЛЬКО если даёт реальную ценность:
   Три критерия (хотя бы один должен быть):
   а) Экспертиза в рынке — человек ПОНИМАЕТ боли клиентов в этой сфере
      Пример: ГЛОНАСС → знает автопарки, их проблемы → может продавать туда
   б) Контакты/связи — есть реальные люди которым можно продать
      Пример: работал в стройке → знает подрядчиков → может им предложить
   в) Техническая компетенция — умеет что-то узкое и редкое
      Пример: ЧПУ → понимает производственные процессы

   Если прошлая работа НЕ даёт ни одного из трёх — ИГНОРИРУЙ её:
   - Грузчик, курьер, таксист, кассир — нет экспертизы, контактов, компетенции
   - НЕ делай "боты для таксистов" только потому что человек таксовал
   - НЕ делай "автоматизация грузоперевозок" потому что был грузчиком

   Если даёт — используй как ЗНАНИЕ АУДИТОРИИ, не как отдельную нишу:
   - ГЛОНАСС + боты → "Telegram-боты для контроля автопарков" (знаешь рынок + текущий навык)
   - НЕ "Установка ГЛОНАСС-систем" (возврат к старому)

3. РАЗНООБРАЗИЕ — разные АУДИТОРИИ, не разные сферы:
   - Если человек хочет делать ботов → 10 ниш = 10 разных аудиторий/применений ботов
   - НЕ НАДО: "ниша 1 — боты, ниша 2 — ЧПУ, ниша 3 — такси" (возврат к старому)
   - НАДО: "ботов для психологов, ботов для автопарков, ботов для маркетплейсов" (разные клиенты)
   - Прошлые работы используй как ЭКСПЕРТИЗУ в аудитории: "ты знаешь автопарки изнутри → можешь делать для них ботов"

4. НИКОГДА не предлагай нишу из прошлой работы как основную, если человек:
   - Указал что работа ему не нравилась
   - Пришёл с другой идеей
   - Явно хочет сменить сферу
